...
doublet_time = .25;
[time,aircraft_state] = ode45(@(time,aircraft_state) GrantsAircraftEOMDoublet(time, aircraft_state, aircraft_surfaces, doublet_size, doublet_time, wind_inertial, density, aircraft_parameters),time,aircraft_state_array);
aircraft_state_vector = aircraft_state';
for i=1:height(time)
    if (0<time(i)) && (time(i)<=doublet_time)
        aircrafted_surfaces = aircraft_surfaces(1) +doublet_size;
    elseif (time(i)>doublet_time) && (time(i)<=2*doublet_time)
        aircrafted_surfaces = aircraft_surfaces(1) -doublet_size;    
    else
        aircrafted_surfaces = aircraft_surfaces(1);
    end
    control_input_array(:,i) = [aircrafted_surfaces;0;0;0.3182]; 
end
PlotAircraftSim(time,aircraft_state_vector,control_input_array,7:12,'r-');